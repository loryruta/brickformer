...
__global__ void
internal::eval_brick_size_kernel(const Placement* placements, size_t num_placements, int* out_brick_sizes)
{
...
__global__ void internal::eval_num_connectible_sides_kernel(const Placement* placements,
                                                            size_t num_placements,
                                                            int* out_num_connectible_sides)
{
...
__global__ void internal::eval_color_map_coverage_kernel(const Placement* placements,
                                                         size_t num_placements,
                                                         ColorMapT* color_map,
...
/// Evaluate the number of neighboring sides every placement would have according to the current placement map. A high
/// number of neighbors is usually better. Can be normalized using the number of `connectible sides` (i.e. same scale).
__global__ void internal::eval_num_neighbors_kernel(const Placement* placements,
                                                    size_t num_placements,
                                                    ...
                                                    int* out_num_neighbors)
{
    ...
    iterate_brick_grid([&](int bx, int by) {
        ...
        if (brick[by][bx] && current_placement_map->is_valid_pixel(mx, my)) {
            ...
            has_left =
                has_left && mx - 1 >= 0 && current_placement_map->read_pixel(mx - 1, my).x != ARP_NO_PLACEMENT_VALUE;
            ...
            has_top =
                has_top && my - 1 >= 0 && current_placement_map->read_pixel(mx, my - 1).x != ARP_NO_PLACEMENT_VALUE;
...
/// Evaluate the number of underlying bricks every placement connects. A high number of connected bricks is usually
/// better. Can be normalized using the `brick size`.
__global__ void internal::eval_num_connected_bricks_kernel(const Placement* placements,
                                                           size_t num_placements,
...
__device__ bool is_outside_or_overlapping(const Placement& placement, const PlacementSolver::Input* input)
{
    ...
    iterate_brick_grid([&](int bx, int by) {
        if (brick[by][bx]) {
            ...
            bool overlapping =
                !out_of_bounds && input->current_placement_map_d->read_pixel(mx, my).x != ARP_NO_PLACEMENT_VALUE;
...
__global__ void
internal::eval_reward_kernel(const PlacementSolver* self, const PlacementSolver::Input* params, float* out_rewards)
{
    int pi = get_placement_index();
...
PlacementSolver::PlacementSolver(size_t num_placements, int resolution, cudaStream_t stream)
    : m_num_placements(num_placements), m_resolution(resolution), m_num_blocks(div_ceil<int>(num_placements, 32))
{
    CHECK_CU(cudaMallocAsync(&m_placements_d, num_placements * sizeof(Placement), stream));
    CHECK_CU(cudaMallocAsync(&m_brick_size_d, num_placements * sizeof(int), stream));
    CHECK_CU(cudaMallocAsync(&m_num_connectible_sides_d, num_placements * sizeof(int), stream));
    CHECK_CU(cudaMallocAsync(
        &m_color_map_coverage_results_d, num_placements * sizeof(internal::ColorMapCoverageResult), stream));
    CHECK_CU(cudaMallocAsync(&m_num_neighbors_d, num_placements * sizeof(int), stream));
    CHECK_CU(cudaMallocAsync(&m_num_connected_bricks_d, num_placements * sizeof(int), stream));
    CHECK_CU(cudaMallocAsync(&m_highest_proximity_d, num_placements * sizeof(int), stream));
    CHECK_CU(cudaMallocAsync(&m_rewards_d, num_placements * sizeof(int), stream));

    internal::init_placements_kernel<<<div_ceil<int>(num_placements, 1024), 1024, 0, stream>>>(
        m_placements_d, num_placements, resolution);
    internal::eval_brick_size_kernel<<<m_num_blocks, 1024, 0, stream>>>(m_placements_d, num_placements, m_brick_size_d);
    internal::eval_num_connectible_sides_kernel<<<m_num_blocks, 1024, 0, stream>>>(
        m_placements_d, num_placements, m_num_connectible_sides_d);
}
...
std::pair<Placement, float> PlacementSolver::solve(const Input& input, cudaStream_t stream)
{
    ...
    PlacementSolver* self_d = to_device(*this, stream);
    Input* params_d = to_device(input, stream);
    ...
    CHECK_CU(cudaMemcpyAsync(
        &placement, &m_placements_d[max_reward_idx], sizeof(Placement), cudaMemcpyDeviceToHost, stream));
    CHECK_CU(cudaMemcpyAsync(&reward, max_reward_d, sizeof(float), cudaMemcpyDeviceToHost, stream));
    CHECK_CU(cudaStreamSynchronize(stream)); // Host read-back

    ARP_DEBUG("Best placement found; PID: %5d, BID: %2d, X: %3d, Y: %3d, Reward: %.3f",
              max_reward_idx,
              placement.bid,
              placement.x,
              placement.z,
