...
void Slicer::slice(uint32_t slice_y, SliceT& out_slice, cudaStream_t stream)
{
    ...
    int32_t* tot_intersections_d = to_device(0, stream);
    ...
    //  Hell nah, that becomes too complex. Consider leveraging graphics pipeline for 3D conservative rasterization!
