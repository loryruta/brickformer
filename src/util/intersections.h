...
struct Triangle {
    ...
    __host__ __device__ glm::vec3 min() const { return glm::min(m_a, glm::min(m_b, m_c)); }

    __host__ __device__ glm::vec3 max() const { return glm::max(m_a, glm::max(m_b, m_c)); }
};

struct Box {
    glm::vec3 m_min, m_max;

    __host__ __device__ glm::vec3 centroid() const { return (m_min + m_max) / 2.0f; } // TODO is it called "centroid"?
};

__host__ __device__ inline bool intersect_box(const Box& a, const Box& b)
{
    // Source:
...
project(const glm::vec3* points, size_t num_points, const glm::vec3& axis, float& out_min, float& out_max)
{
    out_min = INFINITY;
    out_max = -INFINITY;
...
__host__ __device__ inline bool intersect_triangle(const Box& box, const Triangle& triangle)
{
    // Source:
    ...
    const glm::vec3 k_tri_edges[]{
        triangle.m_b - triangle.m_a, triangle.m_c - triangle.m_b, triangle.m_a - triangle.m_c};
    ...
    for (size_t i = 0; i < 3; i++) {
        ...
        if (triangle_max < box.m_min[i] || triangle_min > box.m_max[i]) return false; // Separating axis exists
    }
    ...
    if (box_max < triangle_min || box_min > triangle_max) return false; // Separating axis exists
...
__host__ __device__ inline bool intersect_triangle2(const Box& box, const Triangle& tri)
{
    // Source:
    ...
    const glm::vec3 k_tri_edges[]{tri.m_b - tri.m_a, tri.m_c - tri.m_b, tri.m_a - tri.m_c};
    ...
    glm::vec3 c = glm::vec3(n.x > 0.0f ? dp.x : 0.0f, n.y > 0.0f ? dp.z : 0.0f, n.z > 0.0f ? dp.z : 0.0f);

    auto d1 = glm::dot(n, c - tri.m_a);
    auto d2 = glm::dot(n, dp - c - tri.m_a);

    if ((glm::dot(n, p) + d1) * (glm::dot(n, p) + d2) > 0.0f) return false;
    ...
    if ((glm::dot(ne0xy, pxy) + de0xy) < 0.0f || (glm::dot(ne1xy, pxy) + de1xy) < 0.0f ||
        (glm::dot(ne2xy, pxy) + de2xy) < 0.0f)
        return false;
    ...
    if ((glm::dot(ne0yz, pyz) + de0yz) < 0.0f || (glm::dot(ne1yz, pyz) + de1yz) < 0.0f ||
        (glm::dot(ne2yz, pyz) + de2yz) < 0.0f)
        return false;
    ...
    if ((glm::dot(ne0zx, pzx) + de0zx) < 0.0f || (glm::dot(ne1zx, pzx) + de1zx) < 0.0f ||
        (glm::dot(ne2zx, pzx) + de2zx) < 0.0f)
        return false;
